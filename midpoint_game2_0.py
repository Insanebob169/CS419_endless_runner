...
class GameLoop:
    ...
    def menu(self, name):
        ...
        while(True):
            ...
            self.stdscr.addstr(9, 10, "Instructions - Press 4")
            self.stdscr.addstr(11, 10, "Exit - Press 5")
            self.stdscr.refresh()

            ### Menu logic set up such that host gets priority of commands in unlikely
            ### event that host and guest enter a command in the same loop.
            if s1.status == "host":
                choice = self.stdscr.getch()
                self.choice = str(choice)
                s1.sendInput(self.choice)
                ...
                elif otherChoice == 49: #choice is 1
                    self.player = 1
                    return True
                elif otherChoice == 50:
                    self.player = 0
                ...
                #elif choice == 51:
                #       call highscores menu
                elif otherChoice == 52: #choice is 4
                    self.instructions()
                elif otherChoice == 53: #choice is 5
                    return False

            elif s1.status == "guest":
                choice = self.stdscr.getch()
                self.choice = str(choice)
                s1.sendInput(self.choice)
                otherChoice = s1.otherInput()
                otherChoice = int(otherChoice)
                if otherChoice == 49: #choice is 1
                    self.player = 1
                    return True
                elif otherChoice == 50:
                    self.player = 0
